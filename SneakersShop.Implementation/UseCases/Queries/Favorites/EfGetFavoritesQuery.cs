...
public class EfGetFavoritesQuery(SneakersShopDbContext context, IApplicationUser? user) : EfUseCase(context, user), IGetFavoritesQuery
{
    public int Id => 32;
    ...
    public PagedResponse<ProductsDto> Execute(FavoriteSearch search)
    {
        ...
        var favoriteProductColorIds = Context.Favorites
            .Where(f => f.UserId == User.Id)
            .Select(f => f.ProductColorId)
            .ToList();

        var products = Context.ProductColors
            .Include(x => x.Product)
        ...
        var response = new PagedResponse<ProductsDto>
        {
            ...
            Data = products.Skip(skip).Take(search.PerPage.Value)
                .AsEnumerable()
                .Select(x =>
                {
                    ...
                    return new ProductsDto
                    {
                        ...
                        ReviewCount = x.Product.Reviews.Any() ? x.Product.Reviews.Count() : 0,
                        OldPrice = oldPrice,
                        NewPrice = newPrice,
